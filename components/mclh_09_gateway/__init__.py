    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_MAX_DEVICES, default=4): cv.int_,
...
async def to_code(config):
    var = cg.new_Pvariable(config[CONF_ID], config[CONF_MAX_DEVICES], config[CONF_INTERVAL], config[CONF_ERROR_COUNTING], config[CONF_RAW_SOIL])
    ...
    mac_fields = (0 << 0) | (0 << 8) | (entity_helpers.register_icon("mdi:bluetooth") << 16) | (0 << 24) | (0 << 25) | (0 << 26)
    cg.add(var.set_sensor_fields(batt_fields, temp_fields, lumi_fields, soil_fields, humi_fields, rssi_fields, error_fields, alert_fields, mac_fields))

    from esphome.core import CORE
    num_sensors = config[CONF_MAX_DEVICES] * 7
    CORE.platform_counts.setdefault("sensor", 0)
    CORE.platform_counts["sensor"] += num_sensors

    num_selects = config[CONF_MAX_DEVICES] * 2
